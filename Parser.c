...
void ELC_parseTokenVector(TokenVector tokens)
{
    ...
    for (size_t i = 0; i < tokens.size; i++) {
        ...
        case ASSIGN: {
            ...
            if (i - 1 < 0 || tokens.tokens[i - 1].type != IDENTIFIER) {
                printf("%s:%d:%d: ERROR: Expected identifier before `=`\n", "<CHANGE_IT>", curTok.line, curTok.col);
                exit(71);
            }

            if (i - 2 < 0 || (tokens.tokens[i - 1].type != NILLABLE && tokens.tokens[i - 1].type != IDENTIFIER)) {
            ...
            if (!nillable) {
                text = tokens.tokens[i - 2].text;
            } else {
                text = tokens.tokens[i - 3].text;
            }
